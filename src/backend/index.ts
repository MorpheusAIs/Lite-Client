...
import { initaliseMorpheus, Morpheus } from './services/morpheus';
...
declare global {
  namespace NodeJS {
    interface Global {
      retriever: any; // Use specific type if available
      chain: any; // Use specific type if available
    }
  }
}
...
app.on('ready', async () => {
  await createWindow();
  await initaliseMorpheus() //ensure retreiver and chain are eagerly available
